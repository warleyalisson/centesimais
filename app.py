...
def analises_finalizadas(usuario):
    ...
    filtro_param = st.selectbox("Filtrar por parâmetro", ["Todos", "Umidade", "Cinzas", "Proteínas", "Lipídios", "Fibras Totais", "Carboidratos (por diferença)"])
    ...
    with st.expander("🧹 Excluir análise"):
        id_excluir = st.number_input("ID da análise a excluir", step=1)
    ...
    with st.expander("📝 Editar análise"):
        id_editar = st.number_input("ID da análise a editar", step=1, key="editar_id")
...
def modulo_relatorios(usuario):
    ...
    aba = st.radio("Escolha a opção:", ["Exportar Todas as Análises", "Exportar por Tipo de Análise"])
...
def exportar_por_parametro(usuario):
    ...
    escolha = st.selectbox("Selecione o parâmetro:", parametros)
...
def modulo_anotacoes(usuario):
    ...
    anotacoes = pd.read_sql_query(
        "SELECT id, titulo, texto, data FROM anotacoes WHERE usuario_id = ? ORDER BY data DESC",
        conn, params=(usuario['id'],)
    )

    with st.expander("➕ Nova anotação"):
        titulo = st.text_input("Título")
        texto = st.text_area("Conteúdo da anotação")
        if st.button("Salvar anotação"):
            data = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            cursor.execute(
                "INSERT INTO anotacoes (usuario_id, titulo, texto, data) VALUES (?, ?, ?, ?)",
                (usuario['id'], titulo, texto, data)
            )
            conn.commit()
            ...
            with st.expander(f"📝 {row['titulo']} ({row['data']})"):
                ...
                with col1:
                    if st.button("✏️ Editar", key=f"edit_{row['id']}"):
                        novo_texto = st.text_area("Editar anotação", value=row['texto'], key=f"edit_area_{row['id']}")
                        if st.button("Salvar edição", key=f"save_edit_{row['id']}"):
                            cursor.execute("UPDATE anotacoes SET texto = ? WHERE id = ?", (novo_texto, row['id']))
                            conn.commit()
...
def painel_admin():
    ...
    st.subheader("Exportar Dados Administrativos")
    st.download_button(
    ...
    busca = st.text_input("Digite o nome ou parte do nome da amostra")
    if busca:
...
# ---------------------- BLOCO 15: ESTRUTURA PREPARADA PARA NOVAS METODOLOGIAS ----------------------
# Esta função serve como base para métodos futuros, bastando replicar o modelo e ajustar os campos específicos.
...
def nova_metodologia_padrao(nome_parametro, campos, usuario_id):
    ...
    valores = {}
    for campo in campos:
        valores[campo] = st.number_input(f"{campo.replace('_', ' ').capitalize()} (%)", step=0.01)

    if st.button("Calcular e Salvar Metodologia"):
        try:
            dados = list(valores.values())
            media = round(np.mean(dados), 2)
            desvio = round(statistics.stdev(dados), 2) if len(set(dados)) > 1 else 0.0
            coef_var = round((desvio / media) * 100, 2) if media != 0 else 0.0
            data = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                usuario_id, "Nova Amostra", nome_parametro,
                dados[0] if len(dados) > 0 else 0,
                dados[1] if len(dados) > 1 else 0,
                dados[2] if len(dados) > 2 else 0,
                media, desvio, coef_var, data))
            conn.commit()
...
# ---------------------- BLOCO 15: INICIALIZAÇÃO GERAL E ROTEAMENTO ----------------------
